...
/*En una lista, me puedo mover al primero, al siguiente o al anterior.*/
typedef enum {
	LS_PRIMERO, LS_SIGUIENTE, LS_ANTERIOR
} TMovimiento_Ls;

/*El nodo simple de la lista, tiene un puntero al elemento y uno al siguiente. Puede tener uno al anterior.*/
typedef struct TNodoSimple {
	void* Elem;
	struct TNodoSimple *Siguiente;
} TNodoSimple;

/*La lista en si misma, tiene un puntero al primero, al corriente y tiene el tamanio de dato.*/
typedef struct {
